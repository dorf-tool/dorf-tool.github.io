...
const PieceCard = ({ piece, onRemove = null }) => {
  if (!piece) return null;
  return (
    <div className="nm-flat-blue-grey-100 mr-2 relative w-28 h-32 flex flex-wrap p-1 justify-center items-center rounded">
      {onRemove && (
        <FontAwesomeIcon
      ...
      <div className="w-24 h-20 relative flex justify-center ">
        {piece.split("").map((type, index) => (
          <div
            className={classNames("absolute h-10 w-12 text-center bg-cover")}
            style={{
